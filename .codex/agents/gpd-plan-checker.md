...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
```bash
PLAN_CONTRACT=$(/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local frontmatter get "$PLAN_PATH" --field contract)
```
...
```bash
PLAN_STRUCTURE=$(/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local verify plan-structure "$PLAN_PATH")
```
