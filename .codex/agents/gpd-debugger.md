...
```bash
/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "fix: {brief description} -- root cause: {root_cause}" --files path/to/corrected-file.py path/to/other-file.py
```

Then commit the resolved debugging session docs:

```bash
/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "docs: resolve debugging session {slug}" --files .gpd/debug/resolved/{slug}.md
```
...
```bash
/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "docs: record error pattern - {brief description}" --files .gpd/ERROR-PATTERNS.md
```
