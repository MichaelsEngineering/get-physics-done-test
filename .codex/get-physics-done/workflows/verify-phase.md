...
for plan in "$phase_dir"/*-PLAN.md; do
  /home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local frontmatter get "$plan" --field contract
done
```
...
```bash
for plan in "$phase_dir"/*-PLAN.md; do
  ARTIFACT_RESULT=$(/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local verify artifacts "$plan")
...
```bash
for plan in "$phase_dir"/*-PLAN.md; do
  CHECKS_RESULT=$(/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local verify plan "$plan")
