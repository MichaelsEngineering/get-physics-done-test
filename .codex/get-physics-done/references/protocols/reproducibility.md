...
When you need a machine-readable manifest for submission or review preflight, use `@./.codex/get-physics-done/templates/paper/reproducibility-manifest.md` as the canonical schema/example. In that manifest, stochastic `random_seeds[].computation` values must exactly match `execution_steps[].name`, and `environment.lock_file` should name the real lock file used to pin dependencies (for example `uv.lock`).
