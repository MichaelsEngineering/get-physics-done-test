...
task(
  ...
  readonly=false,
  prompt="First, read ./.codex/agents/gpd-{agent}.md for your role and instructions.\n\n{task_prompt}",
...
5. **Never hardcode runtime-specific paths** — use `./.codex/get-physics-done` for specs assets and `./.codex/agents` for agent prompts, and let the installer project shell `gpd` calls onto the correct runtime bridge.
...
8. **Assign an explicit write scope for every subagent.** Parallel agents must not share writable files. Prefer `apply_patch` for targeted changes, and re-read the file immediately before writing.
