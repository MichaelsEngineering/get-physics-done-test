...
if echo "$ARGUMENTS" | grep -q "\-\-fix"; then
  HEALTH=$(/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local --raw health --fix)
else
  HEALTH=$(/home/qol/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local --raw health)
fi
